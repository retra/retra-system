...
public class GroupingMap {
	...
	private static Log logger = LogFactory.getLog(GroupingMap.class);
	...
	public void add(GroupingKey key, GroupedItem item) {
		if (key.getDimension() != dimension) {
			throw new IllegalArgumentException("Invalid key dimension " + key.getDimension() + ". Expected " + dimension);
		}
		for (int mask = 0; mask <= maxMask; mask++) {
